...
import static top.oasismc.oasisguild.util.MsgSender.*;
...
public final class GuildInfoMenu extends BasicGuildMenu {
    ...
    private void regIcons(String gName, ConfigFile menuFile, Player opener, List<IGuildMember> members, int inventoryLength) {
        ...
        ItemStack guildInfo = getGuildInfoItem(gName, getDataHandler().getPlayerJob(gName, opener.getName()), menuFile);
        ...
        if (pJob >= MEDIUM) {
            ...
            ItemStack admin = getNameOnlyItem("guildInfo.admin.", "WRITABLE_BOOK");
            regIcon(inventoryLength - 8, new GuildMenuIcon(admin, event -> {
                event.getWhoClicked().openInventory(getMenuManager().drawGuildEditMenu((Player) event.getWhoClicked(), gName));
            }));
            ItemStack quit;
            if (pJob >= LEADER)
                quit = getNameOnlyItem("guildInfo.disband.", "BARRIER");
            else
                quit = getNameOnlyItem("guildInfo.quit.", "BARRIER");
            ...
            ItemStack quit = getNameOnlyItem("guildInfo.quit.", "BARRIER");
