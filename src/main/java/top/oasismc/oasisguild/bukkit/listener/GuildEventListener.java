...
import top.oasismc.oasisguild.bukkit.api.event.guild.GuildCreateEvent;
import top.oasismc.oasisguild.bukkit.api.event.guild.GuildDisbandEvent;
import top.oasismc.oasisguild.bukkit.api.event.guild.GuildRenameEvent;
import top.oasismc.oasisguild.bukkit.core.GuildManager;
import top.oasismc.oasisguild.bukkit.util.LoreTools;
...
public final class GuildEventListener implements Listener {
    ...
    public void onCreateGuild(GuildCreateEvent event) {
        ...
        //检查玩家是否已经有公会
        if (getDataManager().getGuildNameByPlayer(creator.getName()) != null) {
        ...
        if (needItem) {
            ...
            if (!LoreTools.hasLore(lore, meta.getLore())) {
                sendMsg(creator, "command.create.needItem");
                event.setCancelled(true);
                return;
            }
